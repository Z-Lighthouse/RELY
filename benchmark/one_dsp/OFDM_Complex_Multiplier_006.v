...
//   - Bit widths adjusted: A=14bit, B=16bit, Out=32bit.
//   - DSP Aux inputs: a=10bit, c=12bit.
...
module OFDM_Complex_Multiplier (
    ...
    // WIDTH CHANGE: Adapted to match DSP Core Inputs 'd' (14-bit) and 'b' (16-bit)
    ...
    // WIDTH CHANGE: Output set to 32 bits to match DSP output
    output reg signed [31:0] cr_out, // Real part of C
    output reg signed [31:0] ci_out, // Imaginary part of C
    ...
    reg signed [31:0] p1, p2, p3, p4; // Registers to store intermediate products

    // --- DSP Core I/O ---
    wire signed [31:0] dsp_out;
    reg  signed [9:0]  dsp_in_a;
    reg  signed [15:0] dsp_in_b;
    reg  signed [11:0] dsp_in_c;
            ...
            case (state)
                ...
                3'd5: // Cycle 5: Feed Cr inputs (P1 - P2), Capture P4
                    begin
                        ...
                        // Map: d = P1 (14-bit), c = -P2 (12-bit)
                        dsp_in_d <= p1[13:0];       // Truncated P1
                        dsp_in_c <= -p2[11:0];      // Truncated -P2
