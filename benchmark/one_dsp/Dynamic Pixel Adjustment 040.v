...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [24:0] adjusted_value;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed({{4{b[12]}}, b[8:0]}) < $signed(a)) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= d[9:0];
            offset     <= c;
        ...
        else if ($signed(a) == $signed({{4{b[12]}}, b[8:0]})) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= d[9:0];
            offset     <= c;
        ...
        else if ($signed(a) > $signed({{4{b[12]}}, b[8:0]}) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= d[9:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {24{1'b1}}}; // Saturation (clamp to max positive value)
