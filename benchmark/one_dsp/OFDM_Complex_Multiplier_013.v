...
//   - Bit widths adjusted: A=12bit, B=11bit, Out=18bit.
//   - State machine optimized for 1-cycle DSP latency.
//   - WARNING: The core logic '& c' (where c is 17-bit) effectively forces 
//     the MSB (bit 17) of the 18-bit output to ZERO.
//
module OFDM_Complex_Multiplier (
    ...
    // WIDTH CHANGE: Adapted to match DSP Core Inputs 'd' (12-bit) and 'b' (11-bit)
    input signed [11:0] ar_in,  // Real part of A -> Maps to 'd'
    input signed [11:0] ai_in,  // Imaginary part of A -> Maps to 'd'
    input signed [10:0] br_in,  // Real part of B -> Maps to 'b'
    input signed [10:0] bi_in,  // Imaginary part of B -> Maps to 'b'
            ...
            case (state)
                ...
                3'd1: // Cycle 1: Feed P1 inputs (Ar * Br)
                    begin
                        ...
                        dsp_in_a <= 9'sd0;
                        dsp_in_b <= br_reg;
                        dsp_in_c <= 17'h1FFFF; // Mask (all 1s for 17 bits)
                        state <= 3'd2;
                    end
                ...
                3'd5: // Cycle 5: Feed Cr inputs (P1 - P2), Capture P4
                    begin
                        ...
                        // DSP Logic: ((d + a) * b) & c
                        // We use: b=1, c=Mask. Result = (d + a).
                        // Map: d = P1 (18-bit), a = -P2 (18-bit)
                        // WARNING: Truncation! d(18->12), a(18->9)
                        dsp_in_d <= p1[11:0];       // Truncated P1
                        dsp_in_a <= -p2[8:0];       // Truncated -P2
                        dsp_in_b <= 11'sd1;         // Multiply by 1
                        dsp_in_c <= 17'h1FFFF;      // Mask
                        state <= 3'd6;
                    end
                ...
                3'd6: // Cycle 6: Feed Ci inputs (P3 + P4), Capture Cr
                    begin
                        ...
                        // Map: d = P3, a = P4
                        dsp_in_d <= p3[11:0];       // Truncated P3
                        dsp_in_a <= p4[8:0];        // Truncated P4
                        dsp_in_b <= 11'sd1;         // Multiply by 1
                        dsp_in_c <= 17'h1FFFF;      // Mask
                        state <= 3'd7;
                    end
...
module PreAdd_Mul_Acc_Unit (
    ...
    input signed [11:0] d,
    input clk,
    output signed [17:0] out // Output port added to match assignment
);
