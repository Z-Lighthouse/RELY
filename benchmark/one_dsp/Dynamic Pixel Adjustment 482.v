...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [32:0] adjusted_value;
    reg signed [14:0] alpha;
    reg signed [16:0] pixel_in;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed(b) < $signed(a[14:0])) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= b[14:0];
            offset     <= c;
        ...
        else if ($signed(a[14:0]) == $signed(b)) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= b[14:0];
            offset     <= c;
        ...
        else if ($signed(a[14:0]) > $signed(b) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= b[14:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {32{1'b1}}}; // Saturation (clamp to max positive value)
