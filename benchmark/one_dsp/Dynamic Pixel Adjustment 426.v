...
module top_complex #(
    ...
    input signed [12:0] c,
    input clk,
    output signed [19:0] final_out
);

    // ------------------- Internal Signals -------------------
    reg signed [19:0] adjusted_value;
    ...
    always @(posedge clk) begin
        ...
        else if ($signed(a) > $signed({{1{b[9]}}, b[8:0]}) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= b[9:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {19{1'b1}}}; // Saturation (clamp to max positive value)
