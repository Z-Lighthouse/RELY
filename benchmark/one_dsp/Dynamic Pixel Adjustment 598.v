...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [35:0] adjusted_value;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed(b) < $signed(a[16:0])) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
        ...
        else if ($signed(a[16:0]) == $signed(b)) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
        ...
        else if ($signed(a[16:0]) > $signed(b) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {35{1'b1}}}; // Saturation (clamp to max positive value)
