...
//   - Bit widths adjusted: A=18bit, B=8bit, Out=28bit.
//   - State machine optimized for 1-cycle DSP latency.
//   - Final Add/Sub uses ports 'd' (18-bit) and 'a' (17-bit).
//
module OFDM_Complex_Multiplier (
    ...
    // WIDTH CHANGE: Adapted to match DSP Core Inputs 'd' (18-bit) and 'b' (8-bit)
            ...
            case (state)
                ...
                3'd5: // Cycle 5: Feed Cr inputs (P1 - P2), Capture P4
                    begin
                        ...
                        // Map: d = P1 (18-bit), a = -P2 (17-bit)
                        dsp_in_d <= p1[17:0];       // Truncated P1
                        dsp_in_a <= -p2[16:0];      // Truncated -P2
                ...
                3'd6: // Cycle 6: Feed Ci inputs (P3 + P4), Capture Cr
                    begin
                        ...
                        // Map: d = P3 (18-bit), a = P4 (17-bit)
                        dsp_in_d <= p3[17:0];       // Truncated P3
                        dsp_in_a <= p4[16:0];       // Truncated P4
                        dsp_in_b <= 8'sd1;          // Multiply by 1
                        dsp_in_c <= 15'sd0;
                        state <= 3'd7;
                    end
