...
module top_complex #(
    ...
    input signed [8:0] c,
    input clk,
    output signed [25:0] final_out
);

    // ------------------- Internal Signals -------------------
    reg signed [25:0] adjusted_value;
    reg signed [11:0] alpha;
    reg signed [12:0] pixel_in;
    reg signed [8:0] offset;
    reg signed [25:0] temp;
    reg               enable_adj;
    reg               enable_adj_d1;
    reg [2:0]         mode_sel;
    reg signed [25:0] out;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {25{1'b1}}}; // Saturation (clamp to max positive value)
