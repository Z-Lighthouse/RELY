...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [26:0] adjusted_value;
    reg signed [14:0] alpha;
    reg signed [8:0] pixel_in;
    reg signed [7:0] offset;
    reg signed [26:0] temp;
    reg               enable_adj;
    reg               enable_adj_d1;
    reg [2:0]         mode_sel;
    reg signed [26:0] out;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed({{1{b[9]}}, b[8:0]}) < $signed(a)) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= d[14:0];
            offset     <= c;
        ...
        else if ($signed(a) == $signed({{1{b[9]}}, b[8:0]})) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= d[14:0];
            offset     <= c;
        ...
        else if ($signed(a) > $signed({{1{b[9]}}, b[8:0]}) && c[7]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= d[14:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {26{1'b1}}}; // Saturation (clamp to max positive value)
