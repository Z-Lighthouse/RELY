...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [31:0] adjusted_value;
    reg signed [16:0] alpha;
    reg signed [13:0] pixel_in;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed({{3{b[16]}}, b[13:0]}) < $signed(a)) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
        ...
        else if ($signed(a) == $signed({{3{b[16]}}, b[13:0]})) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
        ...
        else if ($signed(a) > $signed({{3{b[16]}}, b[13:0]}) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= b[16:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {31{1'b1}}}; // Saturation (clamp to max positive value)
