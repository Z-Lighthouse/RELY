...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [28:0] adjusted_value;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed({{2{b[12]}}, b[10:0]}) < $signed(a)) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= d[13:0];
            offset     <= c;
        ...
        else if ($signed(a) == $signed({{2{b[12]}}, b[10:0]})) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= d[13:0];
            offset     <= c;
        ...
        else if ($signed(a) > $signed({{2{b[12]}}, b[10:0]}) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= d[13:0];
            offset     <= c;
  ...
  assign out = stage0;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {28{1'b1}}}; // Saturation (clamp to max positive value)
