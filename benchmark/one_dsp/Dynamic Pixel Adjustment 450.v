...
module top_complex #(
    ...
    // ------------------- Internal Signals -------------------
    reg signed [27:0] adjusted_value;
    reg signed [11:0] alpha;
    reg signed [14:0] pixel_in;
    reg signed [16:0] offset;
    reg signed [27:0] temp;
    reg               enable_adj;
    reg               enable_adj_d1;
    reg [2:0]         mode_sel;
    reg signed [27:0] out;
    ...
    always @(posedge clk) begin
        ...
        if ($signed(a) < 0 && $signed(b) < $signed(a[11:0])) begin
            mode_sel   <= 3'b000;
            enable_adj <= 1'b1;
            alpha      <= b[11:0];
            offset     <= c;
        ...
        else if ($signed(a[11:0]) == $signed(b)) begin
            mode_sel   <= 3'b001;
            enable_adj <= 1'b1;
            alpha      <= b[11:0];
            offset     <= c;
        ...
        else if ($signed(a[11:0]) > $signed(b) && c[8]) begin
            mode_sel   <= 3'b011;
            enable_adj <= 1'b1;
            alpha      <= b[11:0];
            offset     <= c;
  ...
  assign out = stage1;
    ...
    always @(posedge clk) begin
        case (mode_sel)
            ...
            3'b011: temp <= {1'b0, {27{1'b1}}}; // Saturation (clamp to max positive value)
