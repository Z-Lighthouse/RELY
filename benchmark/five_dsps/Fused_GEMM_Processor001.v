...
module Compute_Intensive_System (
    ...
    input signed [8:0]  A_in_0,       // Matrix A inputs for the GEMM accelerator (Row 0)
    input signed [8:0]  A_in_1,       // Matrix A inputs for the GEMM accelerator (Row 1)
    input signed [15:0] B_in_0,       // Matrix B inputs for the GEMM accelerator (Col 0)
    input signed [15:0] B_in_1,       // Matrix B inputs for the GEMM accelerator (Col 1)
    input signed [15:0] scaling_factor, // Scalar input for the post-processing stage
    
    // Data Output (Bit-width updated to match new core)
    output [28:0] final_result          // Final scalar result after post-processing
);

    // Internal bus for the 2x2 matrix result from the GEMM stage (Flattened).
    wire signed [28:0] c_00;
    wire signed [28:0] c_01;
    wire signed [28:0] c_10;
    wire signed [28:0] c_11;
    ...
    PreAdd_Mul_Acc_Unit post_processor_inst (
        ...
        .a(-c_10[8:0]),   // a <= -C_10 (truncated)
        .b(scaling_factor), // b <= scaling_factor
        .c(c_00[7:0]),    // c <= C_00 (truncated)
        .out(final_result)
    );
...
module GEMM_Accelerator (
    ...
    input rst_n,
    input load_en,
    input signed [8:0]  A_in_0,
    input signed [8:0]  A_in_1,
    input signed [15:0] B_in_0,
    input signed [15:0] B_in_1,
    output signed [28:0] C_out_00,
    output signed [28:0] C_out_01,
    output signed [28:0] C_out_10,
    output signed [28:0] C_out_11
);

    // Internal wiring for systolic data propagation.
    wire signed [8:0]  a_h_wire_0_to_1, a_h_wire_1_to_1; 
    wire signed [15:0] b_v_wire_0_to_0, b_v_wire_1_to_1;
...
//   unit to function as a MAC engine with data forwarding registers.
//
module ProcessingElement (
    input clk,
    input rst_n,
    ...
    input signed [8:0]  a_in,   // Input 'A' from left neighbor
    input signed [15:0] b_in,   // Input 'B' from top neighbor
    
    output reg signed [8:0]  a_out, // Output 'A' to right neighbor
    output reg signed [15:0] b_out, // Output 'B' to bottom neighbor
    output signed [28:0] c_out    // Accumulated result
);

    reg signed [28:0] accumulator;
    wire signed [28:0] mac_out;

    // Instantiate the core compute unit. Configured as a MAC engine (a*b + c).
    PreAdd_Mul_Acc_Unit mac_unit (
        .clk(clk),
        .a(a_in),
        .b(b_in),
        .c(accumulator[7:0]), // Connect to new c width
        .d(11'd0),            // Ground new d width
        .out(mac_out)
    );

    // now gets a value that was calculated from inputs two cycles prior.
