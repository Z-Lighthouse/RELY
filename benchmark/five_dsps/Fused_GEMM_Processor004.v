...
module Compute_Intensive_System (
    ...
    // Data Inputs (Unsigned, flattened from arrays)
    input [8:0]  A_in_0,       // Matrix A inputs for the GEMM accelerator (Row 0)
    input [8:0]  A_in_1,       // Matrix A inputs for the GEMM accelerator (Row 1)
    input [15:0] B_in_0,       // Matrix B inputs for the GEMM accelerator (Col 0)
    input [15:0] B_in_1,       // Matrix B inputs for the GEMM accelerator (Col 1)
    input [15:0] scaling_factor, // Scalar input for the post-processing stage
    
    // Data Output (Unsigned, bit-width updated to match new core)
     ...
     * Note: The subtraction is performed using unsigned, two's complement arithmetic.
     */
    PreAdd_Mul_Acc_Unit post_processor_inst (
        .clk(clk),
        
        // Map function to core inputs: out = ((d + a) * b) + c
        // Connections are updated for the new unsigned port widths.
        .d(c_01[10:0]),   // d <= C[0][1] (truncated)
        .a(-c_10[8:0]),   // a <= -C[1][0] (2's complement)
        .b(scaling_factor), // b <= scaling_factor
        .c(c_00[7:0]),    // c <= C[0][0] (truncated)
        .out(final_result)
    );
...
//
// Description:
//   A 2x2 systolic array for matrix multiplication (C = C + A*B) using unsigned
...
module GEMM_Accelerator (
    ...
    input [15:0] B_in_0,
    input [15:0] B_in_1,
    output [28:0] C_out_00,
    output [28:0] C_out_01,
    output [28:0] C_out_10,
    output [28:0] C_out_11
);

    // Internal wiring for systolic data propagation.
    wire [8:0]  a_h_wire_0_to_1, a_h_wire_1_to_1; 
    wire [15:0] b_v_wire_0_to_0, b_v_wire_1_to_1;
...
module ProcessingElement (
    ...
    input [8:0]  a_in,   // Input 'A' from left neighbor
    input [15:0] b_in,   // Input 'B' from top neighbor
    
    output reg [8:0]  a_out, // Output 'A' to right neighbor
    output reg [15:0] b_out, // Output 'B' to bottom neighbor
    output [28:0] c_out    // Accumulated result
);
...
//   operation. This version has a two-stage pipeline and uses unsigned arithmetic.
