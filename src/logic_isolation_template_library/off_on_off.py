...
def create_port_info(all_signals, signal_dict, container_node=None, loop_var=None, original_code_str=""):
    """
    Build an enhanced port_info dict based on signals and context, detecting array signals if applicable.
    """
    port_info = {}
    
    # infer output signals
    ...
    # identify indexed array signals in for-loop context
    indexed_signals = set()
...
def extract_multiplication_info(container_node, signal_dict, param_dict, codegen):
    ...
    try:
        ...
        # accurately rebuild code_body using RenamingCodegen
        ...
        print(f"Error during multiplication info extraction at line {getattr(container_node, 'lineno', 'N/A')}: {e}")
        return None

    
def handle_assign_multiplication(container_node, op_node, module, mod_name, modified_filename,
                                 signal_dict, file_lines, out_dir, param_dict, file_extraction_dict, source_name, module_instance_counts=None):
    try:
        # check if entire module is just a single multiplication assign
        behavioral_items = [
        ...
        if len(behavioral_items) == 1 and behavioral_items[0] == container_node:
            print(f"Extracting entire module {module.name} (single mult assign)")
        ...
        # handle assignments inside for-loops
        if is_inside_for:
            if dsp_count > 0:
                print(f"Assign statement inside for-loop. Estimated DSP usage: {dsp_count}")
                ...
                replacement_code = (
                    f"generate\n"
                    f"  // For-loop logic extracted to module '{mod_name}'\n"
                ...
                # find outermost for/generate container for replacement
                ...
                print("Assign is inside a for-loop, but failed to parse loop count. Skipping.")
                return False 
        
        else: 
            print("Assign is not in a for-loop. Extracting single mult core.")
            
            extraction_result = extract_multiplication_info(
                container_node,  
                signal_dict, 
                param_dict, 
                codegen
            ...
            if not extraction_result:
                print("Node does not match the A*B pattern. Skipping.")
                return False 

            print("Identified pure multiplication pattern A*B.")
            ...
            with open(os.path.join(out_dir, f"{mod_name}.v"), 'w') as f:
                f.write(mod_code)
            print(f"Generated module: {mod_name}.v")
        ...
        print(f"Error during handle_assign_multiplication at line {container_node.lineno}: {e}")
        import traceback
        traceback.print_exc()
        return False


def is_sequential_always(always_node):
    """Check if the block is sequential (posedge/negedge triggered)."""
    if not isinstance(always_node, Always):
        return False
...
def is_comb_always(always_node):
    """Check if the block is a combinational always @(*)."""
    if not isinstance(always_node, Always):
        return False

    sens_list = always_node.sens_list
    return isinstance(sens_list, SensList) and len(sens_list.list) == 1 and sens_list.list[0].type == 'all'


def is_simple_always_mult(always_node):
    """
    Check if always block strictly matches a "single pure A*B + pure pipeline" structure.
    """
    ...
    def is_pure_mult_pattern(node):
        if not isinstance(node, Times):
            return False
        return isinstance(node.left, (Identifier, Pointer)) and \
               isinstance(node.right, (Identifier, Pointer))
    ...
    for stmt in statements:
        ...
        if is_pure_mult_pattern(rhs):
            if core_op_stmt is not None: 
                print("is_simple_always_mult failed: found a second core computation.")
                return False, None
            core_op_stmt = stmt
        elif isinstance(rhs, (Identifier, Pointer)):
            pipeline_stmts.append(stmt)
        else:
            print(f"is_simple_always_mult failed: invalid computation type {type(rhs).__name__}")
            return False, None
                
    if core_op_stmt is None:
        print("is_simple_always_mult failed: core computation not found.")
        return False, None
        
    core_op_node = getattr(core_op_stmt.right, 'var', core_op_stmt.right)
    related_signal_nodes = {core_op_node.left, core_op_node.right}
    
    core_op_output_node = getattr(core_op_stmt.left, 'var', core_op_stmt.left)
    related_signal_nodes.add(core_op_output_node)
    
    # trace dependencies to populate whitelist
    newly_added = True
    while newly_added:
        newly_added = False
        for stmt in pipeline_stmts:
            lhs_node = getattr(stmt.left, 'var', stmt.left)
            rhs_node = getattr(stmt.right, 'var', stmt.right)

            lhs_is_related = lhs_node in related_signal_nodes
            rhs_is_related = rhs_node in related_signal_nodes

            if rhs_is_related and not lhs_is_related:
                related_signal_nodes.add(lhs_node)
                newly_added = True
            if lhs_is_related and not rhs_is_related:
                related_signal_nodes.add(rhs_node)
                newly_added = True

    all_lhs_nodes_in_always = {getattr(s.left, 'var', s.left) for s in statements}
    
    if not all_lhs_nodes_in_always.issubset(related_signal_nodes):
        unrelated_nodes = all_lhs_nodes_in_always - related_signal_nodes
        unrelated_names = [n.name if hasattr(n, 'name') else str(n) for n in unrelated_nodes]
        print(f"is_simple_always_mult failed: found assignment targets unrelated to core computation: {unrelated_names}")
        return False, None

    print("Passed all checks for is_simple_always_mult.")
    return True, core_op_node


def find_mult_outputs(times_node):
    """
    Trace up from a Times node to its assignment to extract the output signal name.
    """
    outputs = []
    rvalue = getattr(times_node, 'parent', None)
    assign_stmt = getattr(rvalue, 'parent', None)

    if isinstance(assign_stmt, NonblockingSubstitution):
        lhs = assign_stmt.left
        var = lhs.var
...
def is_valid_pipelined_mult_module(module, always_node):
    """
    Check if module strictly matches a pure multiplication pipeline structure.
    """
    always_count = 0
    ...
    # all assign statements must be simple wire connections
    all_assign_stmts = [item for item in module.items if isinstance(item, Assign)]
    for assign_stmt in all_assign_stmts:
        rhs_node = getattr(assign_stmt.right, 'var', None)
        if not isinstance(rhs_node, (Identifier, Pointer)):
            return False
     
    is_simple, core_op_node = is_simple_always_mult(always_node)
    if not is_simple:
        return False
    ...
    try:
        if not isinstance(core_op_node, Times):
            return False
        a_node, b_node = core_op_node.left, core_op_node.right
    ...
    # A/B pipeline depth <= DSP internal register limit
    if a_depth > 2 or b_depth > 2: 
        return False

    # output pipeline depth <= DSP output register limit
    if not validate_output_pipeline_depth(always_node, depth_map, core_op_node, max_output_depth=3):
         return False
...
def replace_multiple_lines_with_one(file_lines, stmts_to_replace, instance_code):
    """
    Remove multiple statements and insert new code at the first deletion point.
    """
    if not stmts_to_replace:
        return file_lines 

    lines_to_delete_indices = {stmt.lineno - 1 for stmt in stmts_to_replace}

    # use the first deleted statement's line number
    insert_pos_index = stmts_to_replace[0].lineno - 1
    
    # match original indentation
    original_indent = len(file_lines[insert_pos_index]) - len(file_lines[insert_pos_index].lstrip(' '))
    indented_instance_code = ' ' * original_indent + instance_code.strip() + '\n'

    new_modified_lines = []
    insertion_done = False
    
    for i, line in enumerate(file_lines):
        if i in lines_to_delete_indices:
            if i == insert_pos_index and not insertion_done:
                new_modified_lines.append(indented_instance_code)
                insertion_done = True
            continue
...
def extract_mult_pipeline_slice(always_node, core_op_node, signal_dict, param_dict, codegen):
    """
    Extract a pipeline slice from a complex sequential always block.
    ...
    # decompose always block into reset logic and main data path
        ...
        if if_stmt.false_statement is not None:
            ...
            if temp_reset_name is not None:
                ...
                if contains_conditional_logic(else_block_node):
                    print(f"Slice failure at line {always_node.lineno}: 'else' block contains nested conditional logic.")
                    return None  
                else:
    ...
    if analysis_mode == 'TREAT_AS_WHOLE':
        print("No standard reset structure detected, treating entire block as main logic")
        main_logic_block = always_node.statement
        reset_logic_block = None
        reset_signal_name = None
    elif analysis_mode == 'SEPARATE_RESET_AND_MAIN':
        print("Standard reset structure detected, separating reset and main logic")
        pass
        
    if main_logic_block is None: 
        print(f"Slice error at line {always_node.lineno}: Could not identify main data path block")
        return None
    
    # extract assignments from main data path for flow tracing
    ...
    if not core_stmt: 
        print("Slice error: Could not locate core computation in main data path")
        return None

    core_output_signal = getattr(core_stmt.left.var, 'name', None)
    if not core_output_signal: 
        print("Slice error: Could not determine output signal of core computation")
        return None

    # data flow tracing 
    times_node = core_op_node
    if not isinstance(times_node, Times): return None

    a_node, b_node = times_node.left, times_node.right

    ALLOWED_OPERANDS = (Identifier, Pointer, IntConst, FloatConst)
    if not (isinstance(a_node, ALLOWED_OPERANDS) and isinstance(b_node, ALLOWED_OPERANDS)):
        return None
    
    if not (isinstance(a_node, (Identifier, Pointer)) or isinstance(b_node, (Identifier, Pointer))):
        return None
    ...
    def process_operand(node):
        ...
        if isinstance(node, (Identifier, Pointer)):
            base_name_node = node
            while isinstance(base_name_node, Pointer):
                base_name_node = base_name_node.var
            
            if not isinstance(base_name_node, Identifier):
                return
            
            base_name = base_name_node.name
            
            if base_name in param_dict:
                print(f"Operand '{base_name}' is a parameter, recording it")
                referenced_params.add(base_name)
                return
            
            print(f"Operand '{base_name}' is a signal, starting backward trace")
            ...
            print(f"Operand '{node.value}' is a literal constant, ignoring")
            return
    ...
    # boundary analysis and dependencies
    ...
    if following_assign is not None:
        ...
        print(f"Added assign following the always block to pruning targets: L{assign_lineno_to_remove}")
    else:
        print("No assign immediately following the always block. No extra pruning needed.")

    stmts_to_keep = [s for s in all_assigns_in_always if s not in final_stmts_to_prune]

    # prune AST using line numbers
...
def handle_always_multiplication(container_node, op_node, module, mod_name, modified_filename,
                                 signal_dict, file_lines, out_dir, param_dict, file_extraction_dict,  source_name, module_instance_counts=None):
    """
    Handle A*B patterns in sequential or combinational always blocks.
    """
    if is_always_sequential(container_node):
        if is_valid_pipelined_mult_module(module, container_node):
            try:
                ...
                print(f"Error extracting entire module: {e}")
                return False

        else:
            print(f"Module {module.name} structure is complex. Falling back to pipeline slicing")
    
            slice_result = extract_mult_pipeline_slice(container_node, op_node, signal_dict, param_dict, codegen)
            ...
            if slice_result and slice_result['status'] == 'success':
                ...
                print("Pipeline slicing failed. Skipping to maintain consistency.")
                return False
    
    else: 
        print("Combinational always block detected")
        
        if is_pure_combinational_module(module, container_node, op_node):
            try:
                print(f"Extracting entire module {module.name} (pure combinational mult)")
                ...
                print(f"Error during full combinational module extraction: {e}")
                return False
            ...
            try:
                ...
                with open(os.path.join(out_dir, f"{mod_name}.v"), 'w') as f:
                    f.write(new_mod_code)
                print(f"Generated pure mult core module: {mod_name}.v")
                ...
                original_output_name = output_port_info['connect_to']
                intermediate_wire_name = f"{original_output_name}_core_out"
                wire_width = output_port_info['width']
                wire_signed_str = "signed" if output_port_info.get('signed', False) else ""
                ...
                print(f"Error handling pure mult in combinational always block: {e}")
                import traceback
                traceback.print_exc()
                return False            


def process_multiplication_in_statement(stmt_node, mult_op_items, signal_dict, param_dict, container_node=None, processed_nodes=None, matched_line_numbers=None):
    """
    Process A * B pattern statements.
    """
    ...
    if isinstance(stmt_node, (BlockingSubstitution, NonblockingSubstitution)):
        ...
        if hasattr(stmt_node, 'rightlist') and hasattr(stmt_node.rightlist, 'list') and len(stmt_node.rightlist.list) == 1:
            ...
            print("Using 'rightlist' structure")
        elif hasattr(stmt_node, 'right') and stmt_node.right is not None:
            rhs_node = stmt_node.right
            print("Using 'right' structure")
            
    if rhs_node is None:
        print("Unable to retrieve RHS node")
        return
    
    mult_op = rhs_node.var if isinstance(rhs_node, Rvalue) else rhs_node
    print(f"Top-level op type: {type(mult_op)}")
    
    if not isinstance(mult_op, Times):
        print(f"Not a mult operation: {type(mult_op)}")
        return
    
    print("Found multiplication operation")
    
    left_op, right_op = mult_op.left, mult_op.right
    
    ALLOWED_OPERANDS = (Identifier, Pointer, IntConst, FloatConst)
    left_ok = isinstance(left_op, ALLOWED_OPERANDS)
    right_ok = isinstance(right_op, ALLOWED_OPERANDS)
    
    if not (left_ok and right_ok):
        print(f"Unsupported operand types: left={type(left_op)}, right={type(right_op)}")
        return
    
    print("Operand type check passed")
    
    left_val = eval_expr(left_op, param_dict)
    right_val = eval_expr(right_op, param_dict)
    
    if left_val is not None and right_val is not None:
        print("Both operands are constants, ignoring")
        return
    
    if left_val == 0 or right_val == 0:
        print("Mult by zero detected, ignoring")
        return
    
    if left_val == 1 or right_val == 1:
        print("Mult by one detected, ignoring")
        return
    
    left_width = get_operand_width(left_op, signal_dict, param_dict)
    right_width = get_operand_width(right_op, signal_dict, param_dict)
    
    if left_width is None or right_width is None:
        print("Failed to get bitwidths")
        return
    
    if not (left_width <= 27 and right_width <= 27):
        print("Bitwidth exceeds DSP limit (27 bits)")
        return
    
    print("A*B Pattern Checks Passed")
    
    key_node = container_node if container_node is not None else stmt_node
    if key_node not in mult_op_items:
        mult_op_items[key_node] = []
    mult_op_items[key_node].append(mult_op)


def off_on_off(verilog_path, ast, signal_dict, param_dict, out_dir, 
             module_extraction_counters, file_extraction_dict,
             processed_nodes, instance_hierarchy=None, top_module_name=None, 
             module_instance_counts=None,matched_line_numbers=None, **kwarg):
    ...
    for module in description.definitions:
        ...
        for item in module.items:
            ...
            if isinstance(item, Assign):
                process_multiplication_in_statement(
                    item, 
                    mul_items, 
                    signal_dict, 
                    param_dict, 
                ...
                # skipping return statements in functions for now
                print(f"Found function definition '{item.name}', analysis not yet implemented.")

            elif isinstance(item, Task):
                procedural_assigns = extract_procedural_assigns(item.statement, ...)
                for assign_stmt in procedural_assigns:
                    process_multiplication_in_statement(assign_stmt, ..., container_node=item)
        ...
        if not mul_items:
            ...
            for container_node, times_node_list in mul_items.items():
                for op_node in times_node_list:
                    
                    success = False
                    source_name_for_dict = "off_on_off"     
                    ...
                    if isinstance(container_node, Assign):
                        ...
                        success = handle_assign_multiplication(
                        ...
                        success = handle_always_multiplication(
                            container_node=container_node,
                            op_node=op_node,
                            module=module,
                            mod_name=mod_name,
